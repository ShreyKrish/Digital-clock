 ...
 int i = 0;
   while(i<E){
    ...
    int minimum=0;
     i = 0;
    while(i<E){
        if(cache[index][i].recent_use<cache[index][minimum].recent_use){
            minimum=i;
        }
        i++;
    }
    ...
    /* ... */
     if (cache[index][minimum].valid){
        (*eviction_count)++;
    }
    /* ... */
    cache[index][minimum].valid=true;
    cache[index][minimum].tag=tag;
    cache[index][minimum].recent_use=program_counter;
}
